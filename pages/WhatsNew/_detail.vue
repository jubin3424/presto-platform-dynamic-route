<template>
  <div>
    <div class="post_title">
      <h2 style="margin-top: 4rem;">{{ this.detail.title }}</h2>
    </div>
    <div class="post_user">
    ...
    <div style="text-align: right;">
      <el-button type="success" @click="goToList">목록보기</el-button>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async getDetail() {
        const getDetailNotice = await this.$axios.$get('/api/notices/' + this.id)
        this.detail = getDetailNotice.notice
      },
      async addComment() {
        await this.$axios.$put('/api/notices/comment/' + this.id,
      ...
      async addComments() {
        await this.$axios.$post('/api/notices/comments/' + this.id,
      ...
      async addReply(index, id) {
        await this.$axios.$post('/api/notices/reply/' + this.id,
      ...
      async deleteReply(id, cid, index) {
        await this.$axios.$post('/api/notices/reply/delete/' + this.id,
